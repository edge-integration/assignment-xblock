...
from ev_sga.tests.common import TempfileMixin, DummyResource
...
class StaffGradedAssignmentMockedTests(TempfileMixin):
    ...
    def make_xblock(self, display_name=None, **kwargs):
        ...
        """
        from ev_sga.sga import EvGradedAssignment as cls
    ...
    @mock.patch('ev_sga.sga._resource', DummyResource)
    @mock.patch('ev_sga.sga.render_template')
    @mock.patch('ev_sga.sga.Fragment')
    def test_student_view(self, fragment, render_template):
        ...
        with mock.patch(
            'ev_sga.sga.EvGradedAssignment.get_submission',
            return_value={}
        ), mock.patch(
            'ev_sga.sga.EvGradedAssignment.student_state',
            return_value={
                'answered': None,
                'annotated': None,
                'answer_allowed': True,
            ...
            assert template_arg == 'templates/ev_graded_assignment/show.html'
            ...
            assert student_state['answered'] is None
            assert student_state['annotated'] is None
            assert student_state['answer_allowed'] is True
            assert student_state['max_score'] == 100
            assert student_state['graded'] is None
            fragment.add_css.assert_called_once_with(
                DummyResource("static/css/ev_sga.css"))
            fragment.initialize_js.assert_called_once_with(
                "EvGradedAssignment")

    @mock.patch('ev_sga.sga._resource', DummyResource)
    @mock.patch('ev_sga.sga.EvGradedAssignment.answer_allowed')
    @mock.patch('ev_sga.sga.EvGradedAssignment.get_score')
    @mock.patch('ev_sga.sga.render_template')
    @mock.patch('ev_sga.sga.Fragment')
    def test_student_view_with_score(self, fragment, render_template, get_score, answer_allowed):
        # pylint: disable=unused-argument
        """
        ...
        answer_allowed.return_value = True
        block.comment = "ok"
        ...
        with self.dummy_upload('foo.txt') as (upload, _):
            with mock.patch(
                ...
                'ev_sga.sga.EvGradedAssignment.student_state', return_value={}
            ), mock.patch(
                'ev_sga.sga.EvGradedAssignment.get_or_create_student_module',
            ...
            with mock.patch(
                ...
                assert context['id'] == 'name'
                student_state = json.loads(context['student_state'])
                assert student_state['answered'] == {'filename': 'foo.txt'}
    ...
    def test_studio_view(self):
        ...
        """
        with mock.patch('ev_sga.sga.StudioEditableXBlockMixin.studio_view') as studio_view_mock:
            block = self.make_xblock()
            block.studio_view()
    ...
    @mock.patch('ev_sga.sga.EvGradedAssignment.get_student_item_dict')
    @mock.patch('ev_sga.sga.EvGradedAssignment.answer_allowed')
    @mock.patch('ev_sga.sga.get_sha1')
    def test_upload_download_assignment(self, get_sha1, answer_allowed, get_student_item_dict):
        # pylint: disable=unused-argument
        """
        ...
        with self.dummy_upload(file_name) as (upload, expected):
            with mock.patch('submissions.api.create_submission') as mocked_create_submission, mock.patch(
                "ev_sga.sga.EvGradedAssignment.file_storage_path",
    ...
    @unpack
    def test_finalize_answered_assignment(
            self, finalized_setting, model_change_expected, answer_allowed, get_student_item_dict
    ):
        """
        Tests that finalize_answered_assignment sets a submission to be finalized
        """
        block = self.make_xblock()
        get_student_item_dict.return_value = {
            "student_id": 1,
        ...
        with mock.patch(
            'ev_sga.sga.Submission.objects.get', return_value=fake_submission_object
        ), mock.patch(
            'ev_sga.sga.EvGradedAssignment.get_submission', return_value=fake_submission_data
        ), mock.patch(
            'ev_sga.sga.EvGradedAssignment.student_state', return_value={}
        ):
            block.finalize_answered_assignment(mock.Mock())
    ...
    @mock.patch('ev_sga.sga.EvGradedAssignment.get_student_module')
    @mock.patch('ev_sga.sga.EvGradedAssignment.is_course_staff')
    @mock.patch('ev_sga.sga.get_sha1')
    def test_staff_upload_download_annotated(self, get_sha1, is_course_staff, get_student_module):
        ...
        with self.dummy_upload(file_name) as (upload, expected), mock.patch(
            "ev_sga.sga.EvGradedAssignment.staff_grading_data",
            return_value={}
    ...
    @mock.patch('ev_sga.sga.EvGradedAssignment.is_course_staff')
    @mock.patch('ev_sga.sga.get_sha1')
    def test_staff_download(self, get_sha1, is_course_staff, get_student_module, answer_allowed):
        """
    ...
    def test_answer_allowed(self, past_due, score, is_finalized_submission, expected_value):
        """
        Tests that answer_allowed returns the right value under certain conditions
        """
        block = self.make_xblock()
        with mock.patch(
            "ev_sga.sga.EvGradedAssignment.past_due",
            return_value=past_due
        ), mock.patch(
            "ev_sga.sga.EvGradedAssignment.get_score",
            return_value=score
        ), mock.patch(
            "ev_sga.sga.is_finalized_submission",
            return_value=is_finalized_submission
        ):
            assert block.answer_allowed(submission_data={}) is expected_value

    @mock.patch('ev_sga.sga.zip_student_submissions')
    @mock.patch('ev_sga.sga.EvGradedAssignment.get_sorted_submissions')
    ...
    def test_prepare_download_submissions(
        ...
        with mock.patch(
            "ev_sga.sga.EvGradedAssignment.is_zip_file_available",
            return_value=is_zip_file_available
        ), mock.patch(
            'ev_sga.sga.EvGradedAssignment.get_real_user',
            return_value=self.staff
        ), mock.patch(
            'ev_sga.utils.default_storage.modified_time',
    ...
    def test_prepare_download_submissions_task_called(
        ...
        with mock.patch(
            ...
            'ev_sga.sga.default_storage.modified_time',
            return_value=datetime.datetime.now()
        ):
            response = block.prepare_download_submissions(None)
            response_body = json.loads(response.body)
    ...
    def test_download_submissions(self, is_course_staff):
        ...
        with mock.patch(
            "ev_sga.sga.get_zip_file_path", return_value=path
        ), mock.patch(
            'ev_sga.sga.EvGradedAssignment.get_real_user',
            return_value=self.staff
        ), mock.patch(
            "ev_sga.sga.get_zip_file_name", return_value=filename
        ):
    ...
    def test_clear_student_state(self):
        ...
        answered_file_path = block.file_storage_path(SHA1, orig_file_name)

        with self.dummy_file_in_storage(answered_file_path) as file_path:
            with mock.patch(
                "ev_sga.sga.submissions_api.get_submissions",
                return_value=[fake_submission]
            ) as mocked_get_submissions, mock.patch(
                "ev_sga.sga.submissions_api.reset_score"
                ...
                # Clearing the student state should also delete the answered file
