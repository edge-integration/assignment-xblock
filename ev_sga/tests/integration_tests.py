...
from ev_sga.constants import ShowAnswer
from ev_sga.sga import EvGradedAssignment
from ev_sga.tests.common import (
    TempfileMixin,
    DummyResource,
    get_sha1,
    is_near_now,
...
@ddt
class EvGradedAssignmentTests(TempfileMixin, ModuleStoreTestCase):
    """
    Create a SGA block with mock data.
    """
    @classmethod
    def setUpClass(cls):
        super(EvGradedAssignmentTests, cls).setUpClass()
        cls.set_up_temp_directory()

    @classmethod
    def tearDownClass(cls):
        super(EvGradedAssignmentTests, cls).tearDownClass()
    ...
    def setUp(self):
        ...
        """
        super(EvGradedAssignmentTests, self).setUp()
    ...
    def make_one(self, display_name=None, **kw):
        ...
        block = EvGradedAssignment(self.runtime, field_data, self.scope_ids)
    ...
    def test_student_view(self, fragment, render_template):
        ...
        self.assertEqual(
            template_arg,
            'templates/ev_graded_assignment/show.html'
        )
        ...
        self.assertEqual(student_state['answered'], None)
        self.assertEqual(student_state['annotated'], None)
        self.assertEqual(student_state['answer_allowed'], True)
    ...
    def test_student_view_with_upload(self, fragment, render_template):
        ...
        context = render_template.call_args[0][1]
        student_state = json.loads(context['student_state'])
        self.assertEqual(student_state['answered'], {'filename': 'foo.bar'})
    ...
    def test_student_view_with_score(self, fragment, render_template):
        ...
        self.assertEqual(student_state['answered'], {u'filename': u'foo.txt'})
        self.assertEqual(student_state['annotated'], None)
        self.assertEqual(student_state['answer_allowed'], False)
        self.assertEqual(student_state['max_score'], 100)
    ...
    def test_finalize_answered_assignment(self):
        """
        ...
        response = block.finalize_answered_assignment(mock.Mock(method="POST"))
    ...
    def test_staff_upload_annotated_state(self):
        ...
        with self.dummy_upload('testa.txt') as (upload, _):
            block.upload_assignment(mock.Mock(params={"assignment": upload}))
        block.finalize_answered_assignment(mock.Mock(method="POST"))
    ...
    def test_enter_grade_fail(self, grade):
        ...
        block = self.make_one()
        fred = self.make_student(block, "fred5", filename='foo.txt')
        with mock.patch('ev_sga.sga.log') as mocked_log:
            block.enter_grade(
                mock.Mock(
                    params={
                        'module_id': fred['module'].id,
    ...
    def test_showanswer(self, is_answer_available):
        ...
        with mock.patch(
            'ev_sga.sga.EvGradedAssignment.answer_available',
    ...
    def test_has_attempted(self):
        """
        A SGA problem is attempted if they answered their submission and it is finalized
        """
        block = self.make_one()
    ...
    def make_test_vertical(self, solution_attribute=None, solution_element=None):
        ...
        return (
            """<vertical display_name="SGA Unit">
              <ev_sga url_name="ev_sga" xblock-family="xblock.v1" display_name="SGA Test 1" {solution_attribute}>
                {solution_element}
              </ev_sga>
