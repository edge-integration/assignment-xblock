...
from ev_sga.constants import ITEM_TYPE
from ev_sga.showanswer import ShowAnswerXBlockMixin
from ev_sga.tasks import (
    get_zip_file_name,
    get_zip_file_path,
    zip_student_submissions
)
from ev_sga.utils import (
    get_sha1,
    utcnow,
...
class EvGradedAssignment(StudioEditableXBlockMixin, ShowAnswerXBlockMixin, XBlock):
    """
    This block defines a Staff Graded Assignment.  Students are shown a rubric
    and invited to upload a file which is then graded by staff.
    """
    ...
    display_name = String(
        ...
        default=_('Ev Graded Assignment'),
        scope=Scope.settings,
        help=_("This name appears in the horizontal navigation at the top of "
               "the page. This is the one we have to change")
    )

    question = String(
        help=_("This is the question the staff asks"),
        display_name=_("Assignment Question"),
        scope=Scope.settings,
    ...
    def add_xml_to_node(self, node):
        ...
        """
        super(EvGradedAssignment, self).add_xml_to_node(node)
    ...
    def student_view(self, context=None):
        ...
        """
        The primary view of the EvGradedAssignment, shown to students
        ...
        fragment.add_css(_resource("static/css/ev_sga.css"))
        fragment.add_javascript(_resource("static/js/src/ev_sga.js"))
        fragment.add_javascript(_resource("static/js/src/jquery.tablesorter.min.js"))
        fragment.initialize_js('EvGradedAssignment')
        return fragment
    ...
    def studio_view(self, context=None):  # pylint: disable=useless-super-delegation
        ...
        return super(EvGradedAssignment, self).studio_view(context)
    ...
    def student_state(self):
        ...
        if submission:
            answered = {"text": submission['answer']['text']}
        else:
            answered = None

        score = self.score
        comment = force_text(self.comment)
        ...
        return {
            ...
            "answer_allowed": self.answer_allowed(submission_data=submission),
            "solution": solution,
        ...
        def get_student_data():
            ...
            for student in students:
                ...
                yield {
                    ...
                    # 'feedback': force_text(state.get("feedback", '')),
    ...
    def get_sorted_submissions(self):
        ...
        for submission in submissions:
            if is_finalized_submission(submission_data=submission):
                assignments.append({
                    'submission_id': submission['uuid'],
                    'filename': submission['answer']["text"],
    ...
    def answer_allowed(self, submission_data=None):
        """
